...
def test_dfu(t91x_dfu):
    ...
    # reset nrf53
    logger.info("Resetting nRF53 after nRF91 DFU")
    reset_device(serial=SEGGER_NRF53)
    wait_until_uart_available(CONNECTIVITY_BRIDGE_UART)

    #reset nrf91
    logger.info("Resetting nRF91 after nRF91 DFU")
    dfu_device(NRF91_APP_UPDATE_ZIP, serial=CONNECTIVITY_BRIDGE_UART, reset_only=True)

    t91x_dfu.uart.start()

    # TODO: Fix the CI issue with nRF91 DFU
    # nrf53 does not rest
    expected_lines = ["Zephyr OS", "Firmware version 3"]
